...
def main():
    ...
    if not label:
        ...
        return
    print(f"[Time] Label detection took {t3 - t2:.2f} seconds")
    ...
    if frame is None or found_detections is None:
        ...
        return
    print(f"[Time] Frame finding and object detection took {t4 - t3:.2f} seconds")
    ...
    t5 = time.time()
    print(f"[Time] Spatial context generation took {t5 - t4:.2f} seconds")
    ...
    t6 = time.time()
    print(f"[Time] Text-to-speech conversion took {t6 - t5:.2f} seconds")

    print("Description spoken successfully.")
    print(f"[Total Time] Overall process took {t6 - start_time:.2f} seconds")
